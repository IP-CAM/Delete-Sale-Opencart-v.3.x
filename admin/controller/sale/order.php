...
class ControllerSaleOrder extends Controller
{
    ...
    protected function getList()
    {
        ...
        foreach ($results as $result) {
            $data['orders'][] = array(
                'winka' => $result['winka'],
                'belge_siparis_no' => $result['belge_siparis_no'],
                'order_id' => $result['order_id'],
                'customer' => $result['customer'],
                'customer_group_id' => $result['customer_group_id'],
                'uyelik_tipi' => $result['uyelik_tipi'],
                ...
                'winka_siparis_ekle' => $this->url->link('winka/winka/siparisaktar', 'user_token=' . $this->session->data['user_token'] . '&order_id=' . $result['order_id'] . $url, true)
            );
        }
...
//		exit;
    ...
    public function info()
    {
        ...
        if ($order_info) {
            ...
            $data['email'] = $order_info['email'];
            $data['telephone'] = $order_info['telephone'];
            $data['winka'] = $order_info['winka'];
            $data['winka_aktar'] = $this->url->link('winka/winka/cariaktar', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $order_info['customer_id'] . $url, true);
			...
			foreach ($products as $product) {
                ...
                if (is_file(DIR_IMAGE . $this->model_sale_order->getOrderProductImage($product['product_id']))) {
                    $image = $this->model_tool_image->resize($this->model_sale_order->getOrderProductImage($product['product_id']), 80, 80);
                } else {
        ...
        foreach ($orders as $order_id) {
            ...
            if ($order_info) {
                ...
                $replace = array(
                    'firstname' => $order_info['payment_firstname'],
                    'lastname' => $order_info['payment_lastname'],
                    'tc_kimlik' => $order_info['tc_kimlik'],
                    'vergi_dairesi' => $order_info['vergi_dairesi'],
                    'vergi_no' => $order_info['vergi_no'],
                    'company' => $order_info['payment_company'],
                    'address_1' => $order_info['payment_address_1'],
                    'address_2' => $order_info['payment_address_2'],
                    'city' => $order_info['payment_city'],
